...
class NormaMachine:
    ...
    def _parse_line(self, line: str):
        ...
        Esta versão ignora comentários (qualquer coisa após um #).
        """
        try:
            # Primeiro, remove qualquer comentário e espaços em branco extras
            line_sem_comentarios = line.split('#')[0].strip()

            # Se a linha ficar vazia, ignore-a
            if not line_sem_comentarios:
                return None, None

            label_part, instr_part = line_sem_comentarios.split(':', 1)
